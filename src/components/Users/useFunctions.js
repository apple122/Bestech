...
export default function useFunctions() {
  ...
  useEffect(() => {
    axios.get(url.Mainurl + url.getusers + '&page=' + nextpage).then((res) => {
      setAPI(res.data.results)
      setCount(res.data.count)
      setPage(res.data.total_pages)
    })

    axios.get(url.Mainurl + url.getusers + '&page=1').then((res) => {
  ...
  function delusers(e) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(url.Mainurl + url.delusers + e.id).then((res) => {
          const Toast = Swal.mixin({
            toast: true,
            position: 'top-end',
            showConfirmButton: false,
